...
export default function AdminOrderDetailPage() {
  ...
  const activeManualPayments = (order.paymentInfo.manualPayments || []).filter((payment) => !payment.voidedAt);
  const manualPaidTotal = activeManualPayments.reduce((sum, payment) => sum + payment.amount, 0);
  const manualBalance = Math.max(0, order.totalAmount - manualPaidTotal);
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-white p-4 sm:p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {order.paymentInfo.method === 'upi' && <div className="mt-4 space-y-3 border-t border-border pt-4"><div className="flex items-center justify-between"><p className="text-sm font-semibold">Manual Payment Log</p>{order.paymentInfo.status !== 'pending' && <button type="button" onClick={handleMarkUnpaid} disabled={markingUnpaid} className="text-xs font-medium text-red-600 hover:underline">{markingUnpaid ? 'Updating…' : 'Mark Unpaid'}</button>}</div><div className="grid grid-cols-2 gap-2 rounded-lg bg-surface p-3 text-xs"><div><p className="text-muted-foreground">Paid</p><p className="font-semibold text-green-700">{formatPrice(manualPaidTotal)}</p></div><div className="text-right"><p className="text-muted-foreground">Balance</p><p className="font-semibold">{formatPrice(manualBalance)}</p></div></div>{(order.paymentInfo.manualPayments || []).length > 0 && <div className="max-h-40 space-y-2 overflow-auto">{order.paymentInfo.manualPayments!.map((payment, index) => <div key={payment._id || index} className={`rounded-lg p-2 text-xs ${payment.voidedAt ? 'bg-gray-100 text-muted-foreground line-through' : 'bg-green-50 text-green-800'}`}><div className="flex justify-between"><span>{new Date(payment.paidAt).toLocaleDateString('en-IN')}</span><strong>{formatPrice(payment.amount)}</strong></div>{payment.reference && <p>Ref: {payment.reference}</p>}{payment.note && <p>{payment.note}</p>}{payment.voidedAt && <p className="no-underline">Voided</p>}</div>)}</div>}<div className="grid grid-cols-2 gap-2"><input type="number" min="0.01" max={manualBalance} step="0.01" value={paymentDraft.amount || ''} onChange={(e) => setPaymentDraft({ ...paymentDraft, amount: Number(e.target.value) })} placeholder="Amount" className="input-field py-2 text-sm" /><input type="date" value={paymentDraft.paidAt} onChange={(e) => setPaymentDraft({ ...paymentDraft, paidAt: e.target.value })} className="input-field py-2 text-sm" /><input value={paymentDraft.reference} onChange={(e) => setPaymentDraft({ ...paymentDraft, reference: e.target.value })} placeholder="Reference / UTR" className="input-field col-span-2 py-2 text-sm" /><input value={paymentDraft.note} onChange={(e) => setPaymentDraft({ ...paymentDraft, note: e.target.value })} placeholder="Note (optional)" className="input-field col-span-2 py-2 text-sm" /></div><button type="button" onClick={handleRecordPayment} disabled={savingPartialPayment || paymentDraft.amount <= 0 || manualBalance <= 0} className="btn-outline btn-sm w-full">{savingPartialPayment ? 'Recording…' : 'Record Payment'}</button></div>}
